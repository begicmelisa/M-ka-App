...
<body>
	...
	if(isset($_POST['postComment' . $post_id])) {
		...
		if($posted_to != $userLoggedIn){
			...
			$notification->inseetNotification($post_id, $posted_to,"comment");
		}
		if($user_to != 'none' && $user_to != $userLoggedIn){
			$notification =new Notification($con,$userLoggedIn);
			$notification->inseetNotification($post_id, $user_to,"profile_comment");
		}
		...
		while ($row =mysqli_fetch_array($get_commenters)) {
			if($row['posted_by'] != $posted_to && $row['posted_by'] != $user_to && $row['posted_by'] != $userLoggedIn && !in_array($row['posted_by'], $notified_users)){
				$notification =new Notification($con,$userLoggedIn);
				$notification->inseetNotification($post_id, $row['posted_by'],"comment_non_owner");
